<script type="text/javascript">
    ...
    function initAutocomplete(count){
        $('#nofaktur'+count).autocomplete("<?= app_base_url('/inventory/search?opsi=nofaktur') ?>",
        {
            extraParams:{
        ...
        $('#barang'+count).autocomplete("<?= app_base_url('/inventory/search?opsi=packing_barang') ?>",
        {
            parse: function(data){
                var parsed = [];
                for (var i=0; i < data.length; i++) {
            ...
            formatItem: function(data,i,max){
                if(data.kategori=='Obat'){
                    ...
                    if(data.kekuatan!=null && data.kekuatan!=0){
                        var str='<div class=result><b>'+data.barang+'</b> <i>'+data.kekuatan+', '+data.sediaan+' @'+data.nilai_konversi+' '+data.satuan_terkecil+'</i>'+pabrik+'</div>';
                    }else{
                        var str='<div class=result><b>'+data.barang+'</b> <i>' +data.sediaan+' @'+data.nilai_konversi+' '+data.satuan_terkecil+'</i>'+pabrik+'</div>';
                    }
                }else{
                    if(data.kekuatan!=null && data.kekuatan!=0){
                        var str='<div class=result><b>'+data.barang+'</b> <i>'+data.kekuatan+', '+data.sediaan+' @'+data.nilai_konversi+' '+data.satuan_terkecil+'</i></div>';
                    }else{
    ...
    $(document).ready(function(){
        $("#tambahBaris").click(function(){
            ...
            "<td align='center'><input  style='width:90%' type='text' name='barang["+counter+"][nama]' id='barang"+counter+"' class='auto'> " +
            ...
            "<td align='center'><input type='text' name='barang["+counter+"][faktur]' id='nofaktur"+counter+"' class='auto' id='nofaktur"+counter+"'></td> " +
            "<td align='center'><input size=8 type='text' name='barang["+counter+"][batch]' id='batch"+counter+"' class='auto'></td> " +
            "<td align='center'><input type='text' size=8 name='barang["+counter+"][jumlah]' class='auto'></td> " +
...
?>
<form action="<?= app_base_url('inventory/control/surat-retur') ?>" method="post" >
    ...
    <table id="tblPemesanan" class="table-input">
        ...
        ?>
            <tr class="barang_tr <?= ($i % 2) ? 'even' : 'odd' ?>">
                ...
                <td align="center"><input type="text" name="barang[<?= $i ?>][nofaktur]" id="nofaktur<?= $i ?>" class="auto"></td>
                <td align="center"><input size=8 type="text" name="barang[<?= $i ?>][batch]" id="batch<?= $i ?>" class="auto"></td>
    ...
    <span class="input-process" style="clear:left;float: left;margin: 10px;">
        <input type="submit" value="Simpan" name="save" class="tombol" /> <input type="button" value="Batal" class="tombol" onClick="javascript:location.href='<?= app_base_url('inventory/surat-retur') ?>'" />
    </span>
</form>
