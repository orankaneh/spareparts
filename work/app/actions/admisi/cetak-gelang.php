    ...
    $(function() {
        ...
        $('#noRm').autocomplete("<?= app_base_url('/admisi/search?opsi=norm_kunjungan') ?>",
        {
		 ...
		 $(".bayar").html(carabayar);
                    ...
                    $.ajax({
                        url: "<?= app_base_url('billing/pasien-discharge-table')?>",
                        cache:false,
    ...
    $(function() {
        $('#pasien').autocomplete("<?= app_base_url('/admisi/search?opsi=kunjungan') ?>",
        {
            ...
            function(event,data,formated){
                    ...
                    var id_pasien = data.pasien;
                    $.ajax({
                        url: "<?= app_base_url('admisi/cetak-gelang-table')?>",
                        cache:false,
                         data: "&norm="+data.pasien+"&no_kunjungan_pasien="+data.no_kunjungan_pasien+"&billing="+data.id_billing+"&idKunjungan="+data.id,
                        success:function(msg){
                            $('#tblRawatInap').html(msg);
...
?>
<h2 class="judul">Cetak Gelang</h2>
<?=  isset ($pesan)?$pesan:''?>
<div class="data-input">   
		<fieldset>
			<legend>Form Cetak Gelang</legend>
                            ...
                            ?>
                              ...
                              <!--    <td align='center'><?//datetime($row['tanggal'])?></td>-->
                               ...
                               -->
                                  ...
                                  <td align='center'>
                                  <? if ($action !='preview'){?>
                                      <span class="discharge" align='center'><a href="<?=app_base_url('admisi/control/discharge?idBilling=').$idBilling."&idKunjungan=".$idKunjungan."&idTarif=".$idTarif?>" onClick="return confirm('Apakah Anda yakin akan mendischarge pasien berikut?')">Discharge</a></span>
