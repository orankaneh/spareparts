...
if (isset($_POST['add'])) {
	_update("update dinamis_penduduk set akhir = 0 where id_penduduk = '$idPenduduk'");
	$sql = _insert("insert into dinamis_penduduk values 
	('',now(),'$idPenduduk','$alamat','$telpon',{$arr[1]},{$arr[2]},{$arr[3]},{$arr[5]},{$arr[6]},1,{$arr[4]})");
	
	if ($sql) {
		header("location: ".app_base_url('admisi/detail-pegawai')."?id=$idPenduduk&msg=1");
	}
}

if (isset($_POST['edit'])) {
	$sql = _update("update dinamis_penduduk set tanggal = now(), alamat_jalan = '$alamat', no_telp = '$telpon', id_kelurahan = {$arr[1]}, status_pernikahan = {$arr[2]}, id_pendidikan_terakhir = {$arr[3]}, id_profesi = {$arr[5]}, id_agama = {$arr[6]}, id_pekerjaan={$arr[4]} where id_penduduk = '$idPenduduk' and akhir=1");
	if ($sql) {
		header("location: ".app_base_url('admisi/detail-pegawai')."?id=$idPenduduk&msg=1");
	}
}
...
<?php
    ...
    $data = $penduduk;
    delete_list_data($_GET['id'], 'dinamis_penduduk', 'admisi/detail-pegawai?id='.$_GET['id'].'&msg=2','admisi/detail-pegawai?id='.$_GET['id'].'&msr=8'," dinamis penduduk ".$data['nama']);
}
?>
