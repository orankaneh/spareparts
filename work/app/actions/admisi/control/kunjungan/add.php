...
$statuspasien      = isset($_POST['status_pasien'])?$_POST['status_pasien']:null;
if (isset($_POST['save'])) {
   if (empty($responsibleName) and empty($responsibleId) and empty($responsibleAddr) and empty($responsiblePhon)) {
$responsibleName = isset($_POST['nama']) ? $_POST['nama'] : null;
$responsibleId = isset($_POST['idPenduduk']) ? $_POST['idPenduduk'] : null;
$responsibleAddr = isset($_POST['alamatJln']) ? $_POST['alamatJln'] : null;
   $telp = _select_unique_result("select * from dinamis_penduduk where id= '$responsibleId'");
$responsiblePhon = isset($telp['no_telp']) ? $telp['no_telp'] : null;
}
...
//        show_array($array);
		//echo 'count pasien: '.$count.' penduduk: '.$countz;
        ...
        if (($count == 0) and ($countz >= 1)) {
            ...
            _update("update dinamis_penduduk set alamat_jalan = '$address', id_kelurahan = $array[1], status_pernikahan = '$array[2]', id_pendidikan_terakhir = $array[3], id_profesi = $array[4], id_agama = $array[5],id_pekerjaan=$array[6] where id_penduduk = '$resident' and akhir = '1'");
        }
        if (($count > 0) and ($countz == 0)) {
            _insert("insert into penduduk values ('','','$name','$sex','$bloodGroup','$dateOfBirth','',null,'')");
            $residentId = mysql_insert_id();
...
echo 'B<br />';/*
       ...
       */
        ...
        if (empty($medicalPersonId) and !empty($medicalPerson)) {
            _insert("insert into penduduk (nama) values ('$medicalPerson')");
        ...
        _update("update dinamis_penduduk set alamat_jalan = '$address', id_kelurahan = $village, status_pernikahan = '$marriage', id_pendidikan_terakhir = $education, id_profesi = $profesi, id_agama = $religion,id_pekerjaan=$pekerjaan where id_penduduk = '$resident' and akhir = '1'");
    ...
    //update penanggung jawab
	 _update("update dinamis_penduduk set alamat_jalan = '$responsibleAddr', no_telp = '$responsiblePhon' where id_penduduk = '$responsibleId' and akhir = '1'");
	  //update keluarga
	  _update("update dinamis_penduduk set alamat_jalan = '$familyAddress', no_telp = '$familyPhoneNumb' where id_penduduk = '$familyId' and akhir = '1'");
	  
	    //update pengirim
		  
	  _update("update dinamis_penduduk set alamat_jalan = '$guideAddr', no_telp = '$guidePhoneNumb' where id_penduduk = '$guideId' and akhir = '1'");
 ...
 if ($familyId == '') {
        ...
        $success = _insert("insert into kunjungan values ('',$new_sequence,now(),'$patientId',$hasil,'$destination','$dokter',$array[1],'$guideId',$array[3],'$_POST[caraPembayaran]','$bed','Masuk')") or die(mysql_error());
		echo $success;
        $admisionId = mysql_insert_id();
        $tarif_kunjungan=_select_unique_result("select total from tarif where id_layanan=1 and id_kelas=1 and status='Berlaku' order by id desc limit 1");
        $tarif_cetak_kartu=_select_unique_result("select total from tarif where id_layanan=2 and id_kelas=1 and status='Berlaku' order by id desc limit 1");
		$tarif_maintance=_select_unique_result("select total from tarif where id_layanan=803 and id_kelas=1 and status='Berlaku' order by id desc limit 1");
...
values (now(),' $patientId','" . User::$id_user . "','0')");
        ...
        if ($statuspasien  == 'baru') {
			...
			$bayar_cetak_kartu=_insert("insert into detail_billing (id_billing,id_tarif,waktu,frekuensi) values
				...
				_insert("insert into detail_billing (id_billing,id_tarif,waktu,frekuensi) values
                ('$idBilling',(select max(id) from tarif where id_layanan=803 and id_kelas=1 and status='Berlaku'),now(),1)");
				
            $total=$tarif_cetak_kartu['total']+$tarif_kunjungan['total']+$tarif_maintance['total'];
            _update("update billing set total_tagihan=$total where id=$idBilling");
        } else if ($statuspasien=='lama') {
            //kunjungan pasien lama
		$tarif_kunjungan_lama=_select_unique_result("select total from tarif where id_layanan=790 and id_kelas=1 and status='Berlaku' order by id desc limit 1");
            _insert("insert into detail_billing (id_billing,id_tarif,waktu,frekuensi) values
                ('$idBilling',(select max(id) from tarif where id_layanan='790' and id_kelas=1 and status='Berlaku'),now(),1)");
				_insert("insert into detail_billing (id_billing,id_tarif,waktu,frekuensi) values
                ('$idBilling',(select max(id) from tarif where id_layanan=803 and id_kelas=1 and status='Berlaku'),now(),1)");
            $total=$tarif_kunjungan_lama['total']+$tarif_maintance['total'];
		    _update("update billing set total_tagihan='$total' where id=$idBilling");
        }
        ...
        if ($success) {
         header('location:' . app_base_url('admisi/kunjungan-info?id=' . $admisionId));
        }
    }
}
?>
