...
$PAGE_TITLE = '<h2 class=judul>Laporan Kunjungan Flat</h2>';
...
$sql= "
    ...
    WHERE bed.id_kelas = 1 and dp.akhir = 1 and date(kj.waktu)>='".date2mysql($startDate)."' and  date(kj.waktu)<='".date2mysql($endDate)."' and kj.id = (select max(id) from kunjungan where kunjungan.id_pasien=p.id) and kj.no_kunjungan_pasien=1";// and kj.status = 'Masuk'";
...
$uri=substr($uri, strlen($page)+29);
 ...
 if(isset ($_GET['Apply'])){ ?>
 <div>
<span class="cetak"><a href="" <?="onclick=window.open('$page"."admisi/informasi/flat-report-rep?$uri','popup','width=1000','height=650')"?>>Cetak</a>
<a class="excel" href="<?=app_base_url("admisi/informasi/excel/flat-report-rep?$uri")?>">Cetak</a></span></div>
<? } ?>
