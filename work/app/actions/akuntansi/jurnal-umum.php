...
switch ($sub) {
	...
	$data = jurnal_umum_muat_data($bulan,$tahun,1);
	...
	<center><h2 class="judul">JURNAL UMUM<br/><?= $profil['nama'] ?><br/>
	...
	echo excelButton("/akuntansi/report/jurnal-umum?bulan=".$bulan."&tahun=".$tahun."&tipe=1",'Cetak Excel');
	?>
	...
	<table class="tabel full" style="margin-top: 10px" border=1 bordercolor='#ccc'>
        ...
        <tr class="<?php echo $warnarekening; ?>" id="<?php echo $row['id']; ?>">
			...
			<td rowspan=<?php echo $rspan; ?> class="aksi" style='border-bottom: 1px solid #ccc !important'>
                ...
                <a href="" onclick="showFormConfirm('Apakah Anda ingin menghapus transaksi <b><?php echo $row['nama']; ?></b> tersebut ?','<?= app_base_url('akuntansi/control/jurnal-umum?opsi=hapus'); ?>','<?=$row['id']; ?>','<?= app_base_url('/akuntansi/jurnal-umum?section=tabeljurnalumum')."&bulan=".$bulan."&tahun=".$tahun; ?>'); return false" class="delete">Delete</a>
            </td>
			<?php 
			$ulang+=1;
			} 
		...
		echo "</table><br>".topBUtton("#atas",'Kembali Ke Atas');
		} else {
			...
			echo notifikasi("Jurnal tidak tersedia pada bulan ini",1);
		}
	...
	if (isset($id)) {
		$data_jurmum=jurnal_umum_muat_data($bulan,$tahun,1,$id);
	...
	<form action="<?php echo $urlsimpan ?>" method="post" onsubmit="simpanJurnal($(this));return false;">
		<fieldset id="master"><legend>Form Transaksi Jurnal Umum </legend>
			...
			<select name="kode" id="kode" class="select-style" style="min-width: 100px;">
		...
		<fieldset><legend>Peminjam / Pemberi Pinjaman <font style="color: #5090BA">( Opsional ) *</font></legend>
			<label for="status_terkait">Status Peminjam</label>
				...
				<script type="text/javascript">
					...
					switch(casepiutang) {
						case 1: contentloader('<?= app_base_url('/akuntansi/jurnal-umum?section=piutangpasien'); ?>&nama=<?php echo isset($nama_terkait)?$nama_terkait:null; ?>&kode=<?php echo isset($idterkait)?$idterkait:null; ?>','#autopiutang'); break;
						case 2: contentloader('<?= app_base_url('/akuntansi/jurnal-umum?section=piutangpegawai'); ?>&nama=<?php echo isset($nama_terkait)?$nama_terkait:null; ?>&kode=<?php echo isset($idterkait)?$idterkait:null; ?>','#autopiutang'); break;
						case 3: contentloader('<?= app_base_url('/akuntansi/jurnal-umum?section=piutanginstansi'); ?>&nama=<?php echo isset($nama_instansi)?$nama_instansi:null; ?>&kode=<?php echo isset($idterkait)?$idterkait:null; ?>','#autopiutang'); break;
					}
		...
		<input type="text" readonly="readonly" id="piutang-plan"/></div>
			<label></label>
				...
				<input type="hidden" name="tipejurnal" value="1">
...
contentloader('<?= app_base_url('/akuntansi/jurnal-umum?section=tabeljurnalumum'); ?>&bulan=<?php echo $bulan; ?>&tahun=<?php echo $tahun; ?>','#content');

function showFormAdd(bulan,tahun) {
	contentloader('<?= app_base_url('/akuntansi/jurnal-umum?section=formjurnalumum'); ?>&bulan='+bulan+'&tahun='+tahun,'#admission');
}

function editJurnalUmum(bulan,tahun,id) {
    contentloader('<?= app_base_url('/akuntansi/jurnal-umum?section=formjurnalumum'); ?>&bulan='+bulan+'&tahun='+tahun+'&id='+id,'#admission');
	$('.even,.odd').hide();
	$('#'+id).show();
...
function piutangSelected(thisval) {
	switch(thisval){
		...
		case "1": contentloader('<?= app_base_url('/akuntansi/jurnal-umum?section=piutangpasien'); ?>','#autopiutang'); break;
		case "2": contentloader('<?= app_base_url('/akuntansi/jurnal-umum?section=piutangpegawai'); ?>','#autopiutang'); break;
		case "3": contentloader('<?= app_base_url('/akuntansi/jurnal-umum?section=piutanginstansi'); ?>','#autopiutang'); break;
		default: contentloader('<?= app_base_url('/akuntansi/jurnal-umum?section=piutangplano'); ?>','#autopiutang'); break;
	}
}
	...
	if ($('#transaksi').val() == '') {
		...
		if (status_kredit == 1) {
				if (totalJurnal != totalDebet) {
					...
					var bulan = $('#bulan').val();
					var tahun = $('#tahun').val();
					setTimeout("contentloader('<?= app_base_url('/akuntansi/jurnal-umum?section=tabeljurnalumum'); ?>&bulan="+bulan+"&tahun="+tahun+"','#content');",1200);
					$('#admission').html('');
...
function tampilJurnalUmum(bulan,tahun) {
	contentloader('<?= app_base_url('/akuntansi/jurnal-umum?section=tabeljurnalumum'); ?>&bulan='+bulan+'&tahun='+tahun,'#content');
	$('#admission').html('');
}
</script>
...
<h1 class="judul"><a href="<?= app_base_url('akuntansi/jurnal-umum') ?>">Transaksi Jurnal Umum</a></h1>
