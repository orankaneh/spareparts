...
if($key != NULL){
      ...
      $cari='';
  $result = array();
   $sql="select * from agama $where $cari order by nama asc";
...
function tarif_muat_data($id = NULL, $page = null, $dataPerPage = null, $key = NULL) {
...
//    $action = " where t.status='Berlaku'";
    ...
    $sql = "select t.*,kt.nama as nama_ktarif, l.nama as layanan,l.bobot,l.jenis,ins.nama as instalasi,sp.id as id_spesialisasi,sp.nama as spesialisasi,pr.nama as profesi,k.nama as kelas from tarif t
        ...
        left join profesi pr on sp.id_profesi = pr.id $action order by t.id,l.nama,k.nama $batas
	";
...
function detail_kunjungan($id){
    ...
    LEFT JOIN dinamis_penduduk dp on dp.id_penduduk=penduduk.id
    ...
    LEFT JOIN dinamis_penduduk dp_penanggungjawab on dp_penanggungjawab.id_penduduk=penanggungjawab.id
    LEFT JOIN dinamis_penduduk dp_pengantar on dp_pengantar.id_penduduk=pengantar.id
...
function barang_muat_data($id=null, $sort = null, $sortBy=null, $page=NULL, $dataPerPage = NULL,$key=NULL, $kategori = NULL) {
    ...
    $sql = "
        ...
        where kb.id='" . User::$pemesanan_barang_role . "' $where $action $sortir $batas";
    ...
    $sqli = "
        ...
        JOIN sub_kategori_barang subkategori on subkategori.id=b.id_sub_kategori_barang
        JOIN kategori_barang kb on kb.id=subkategori.id_kategori_barang
        where kb.id='" . User::$pemesanan_barang_role . "' $where $action";
...
function packing_barang_muat_data($id=null, $page = null, $dataPerPage = null, $sort = NULL,$sortBy=null,$key=null) {
    ...
    $sql = "
        ...
        where kb.id='" . User::$pemesanan_barang_role . "' and pb.barcode !='' $where $dimana $order $batas";
    if ($id != null) {
		$result = _select_unique_result($sql);
        $result['list'] = _select_arr($sql);
    } else {
    ...
    $sqli = "
        SELECT
            pb.id as id_packing,pb.barcode,pb.id_barang,pb.id_satuan_terbesar,
            pb.nilai_konversi,pb.id_satuan_terkecil,b.id as id_barang,b.nama as nama_barang,
        ...
        LEFT JOIN satuan kemasan on kemasan.id=pb.id_satuan_terbesar
        LEFT JOIN satuan on satuan.id=pb.id_satuan_terkecil
        where kb.id='" . User::$pemesanan_barang_role . "'  and pb.barcode !='' $where $dimana";
...
function kategori_barang_muat_data($id=null, $sort = null,$sortBy = null, $page=NULL,$dataPerPage = null, $key = null) {
    ...
    $sql = "SELECT sk.id,sk.permit_penjualan,sk.nama,k.nama as kategori FROM sub_kategori_barang sk
        JOIN kategori_barang k on k.id=sk.id_kategori_barang where k.id='" . User::$pemesanan_barang_role . "' $where $cari $sortir $batas";
       ...
       $result = _select_unique_result($sql);
        ...
        $result['list'] = _select_arr($sql);
            ...
            $sqli = "SELECT sk.id,sk.nama,k.nama as kategori FROM sub_kategori_barang sk
                JOIN kategori_barang k on k.id=sk.id_kategori_barang where k.id='" . User::$pemesanan_barang_role ."' $where $cari";
...
function pembelian_muat_data($startDate=null, $endDate=null, $suplier = null, $pegawai = NULL, $awalJatuhTempo = NULL, $akhiJatuhTempo = NULL,$idpacking=null,$batch) {
    ...
    $sql = "select pb.id,DATE(pb.waktu) as waktu,pb.no_faktur,pb.ppn,dpr.diskon,dpr.harga_pembelian,dpr.jumlah_pembelian,(dpr.jumlah_pembelian*dpr.harga_pembelian) as total,pb.tanggal_jatuh_tempo,pdd.nama as pegawai,pb.materai,ir.nama as suplier,
        ...
        where kb.id = '".User::$pemesanan_barang_role."'
        $where
        GROUP BY pb.id";
    return _select_arr($sql);
}
...
function stok_barang_muat_data2($startDate = NULL,$endDate=null, $unit = NULL, $packing = NULL, $jenisTransaksi = NULL, $subKategori = NULL) {
    ...
    $stok = _select_arr("select date(s.waktu) as tanggal,s.*,b.nama as barang,st2.nama as kemasan,o.kekuatan,pb.nilai_konversi,o.generik,sd.nama as sediaan,st.nama as satuan,ir.nama as pabrik,jt.nama as jenis_transaksi,pb.id as id_packing_barang
                                  from stok s 
...
function stok_obat_muat_data2($startDate, $unit = NULL, $packing = NULL, $jenisTransaksi = NULL, $perundangan=null, $generik=null, $indikasi=null, $ven=null, $ssf=null, $formularium=NULL,$endDate=null) {
    ...
    $sql = "select s.id,DATE(s.waktu) as tanggal,obat.generik,s.id_transaksi,s.batch,s.ed,s.id_unit,s.awal,s.masuk,s.keluar,pb.nilai_konversi,satuan.nama as satuan_terkecil,sat.nama as satuan_terbesar,obat.kekuatan,ins.nama as pabrik,sediaan.nama as sediaan,
           s.sisa,s.id_packing_barang,pb.id,pb.id_barang,pb.id_satuan_terkecil,u.nama as unit,obat.generik,obat.kekuatan,
           st.nama as satuan,j.nama as jenis_transaksi,b.nama as barang,
           (select stok.hpp from stok where stok.id_packing_barang=pb.id and stok.id_jenis_transaksi='2' order by stok.waktu desc limit 0,1) as hpp,
           s.id_packing_barang
...
function stok_obat_pelayanan_muat_data($idPacking = NULL,$perundangan = NULL,$generik = NULL,$formularium = NULL,$indikasi = NULL,$ven = NULL,$ssf = NULL,$idZatAktif=NULL,$getUnit=NULL){
    ...
    $sql = _select_arr("select pb.id as id_packing
           from stok_unit s
           join packing_barang pb on (s.id_packing_barang=pb.id) join unit u on (s.id_unit=u.id)
           join jenis_transaksi j on (s.id_jenis_transaksi=j.id) join satuan st on (pb.id_satuan_terkecil=st.id)
           join barang b on (pb.id_barang=b.id)
    ...
    foreach ($sql as $row){
        $result[] = _select_unique_result("select s.hpp,s.sisa,b.nama as barang,st2.nama as satuan_terbesar,pb.nilai_konversi,o.kekuatan,st1.nama as satuan_terkecil,
                sd.nama as sediaan,ir.nama as pabrik,o.generik,s.id
               ...
               left join sediaan sd on o.id_sediaan = sd.id
               left join instansi_relasi ir on ir.id = b.id_instansi_relasi_pabrik
               where pb.id=$row[id_packing] order by s.id desc,s.waktu desc
               LIMIT 0,1");
    }
    return $result;
}
...
function stok_obat_unit_muat_data2($startDate, $packing = NULL, $jenisTransaksi = NULL, $perundangan=null, $generik=null, $indikasi=null, $ven=null, $ssf=null, $formularium=NULL, $endDate) {
    ...
    $sql = "select s.id,DATE(s.waktu) as tanggal,s.awal,s.masuk,s.keluar,pb.nilai_konversi,satuan.nama as satuan_terkecil,
           ...
           (select stok_unit.hpp from stok_unit where stok_unit.id_packing_barang=pb.id and stok_unit.id_jenis_transaksi='2' order by stok_unit.waktu desc limit 0,1) as hpp,
            ir.nama as pabrik,obat.generik,sediaan.nama as sediaan, obat.kekuatan,s.id_packing_barang,stb.nama as satuan_terbesar
           from stok_unit s
           join packing_barang pb on (s.id_packing_barang=pb.id)
           join jenis_transaksi j on (s.id_jenis_transaksi=j.id)
           join satuan st on (pb.id_satuan_terkecil=st.id)
...
function sp_muat_data($startDate = NULL, $endDate = NULL, $jenisSp = NULL, $idSuplier = NULL, $idPegawai = NULL) {
    ...
    $sql = mysql_query("select p.*,DATE(p.waktu) as tanggal,ir.nama as suplier
           ...
           LEFT JOIN instansi_relasi ir on ir.id=p.id_instansi_relasi_suplier
            ...
            where kategori.id='" . User::$pemesanan_barang_role . "'
...
function barang_adm_muat_data($idPacking=null, $barang=null, $idSubKategori=null, $idKelas=null, $page = NULL,$dataPerPage = NULL) {
    $where = " WHERE kb.id='" . User::$pemesanan_barang_role . "' ";
    if ($idPacking != null) {
        $where .= " AND pb.id='$idPacking'";
...
function asuransi_produk_muat_data($id=null, $sort=NULL) {
    if ($id != null) {
...
function stock_opname_muat_data2($gudang = NULL,$page = NULL,$dataPerPage = NULL) {
    $result = array();
	if($gudang==1){
		$slc=",s.batch,s.ed";
		$st="stok";
		...
		$slc="";
		$st="stok_unit";
                $grup = "group by s.id_packing_barang";
	}
	$unit = $_SESSION['id_unit'];
	if (!empty($page)) {
        $noPage = $page;
    ...
    $sql = "select pb.id as id_packing $slc
           from $st s
           join packing_barang pb on (s.id_packing_barang=pb.id) join unit u on (s.id_unit=u.id)
           join jenis_transaksi j on (s.id_jenis_transaksi=j.id) join satuan st on (pb.id_satuan_terkecil=st.id)
           join barang b on (pb.id_barang=b.id)
           join sub_kategori_barang skb on (b.id_sub_kategori_barang=skb.id) where s.id_jenis_transaksi = 11 and s.id_unit='$unit' $grup 
           order by s.id,s.waktu desc $batas";
    $temp=array();
    ...
    foreach ($stok as $row){
        ...
        $temp[] = _select_unique_result("select 
                s.id,s.id_packing_barang,s.waktu,s.sisa,s.hpp,s.hna $slc,b.nama as barang,pb.nilai_konversi,st.nama as satuan,st12.nama as kemasan, ins.nama as pabrik, o.kekuatan, 
	...
	left join instansi_relasi ins on ins.id=b.id_instansi_relasi_pabrik 
        where s.id_jenis_transaksi = 11 and s.id_unit='$unit' $req2 and pb.id = '$row[id_packing]' order by s.id desc");
    }
    $sqli = "select pb.id as id_packing $slc
           ...
           join jenis_transaksi j on (s.id_jenis_transaksi=j.id) join satuan st on (pb.id_satuan_terkecil=st.id)
           join barang b on (pb.id_barang=b.id)
           join sub_kategori_barang skb on (b.id_sub_kategori_barang=skb.id) where s.id_jenis_transaksi = 11 and s.id_unit='$unit' $grup order by s.id,s.waktu desc";
    $result['paging'] = paging($sqli, $dataPerPage);
    $result['list'] = $temp;
    $result['offset'] = $offset;
    $result['total'] = countrow($sqli);
...
function stock_opname_muat_data($id = NULL,$gudang = NULL) {
    ...
    $sql = "select s.id,s.id_packing_barang,s.waktu,s.sisa,s.hpp,s.hna $slc,b.nama as barang,pb.nilai_konversi,st.nama as satuan, stn.nama as besar 
	from $st s
    left join packing_barang pb on s.id_packing_barang = pb.id
    left join barang b on pb.id_barang = b.id
    left join satuan st on pb.id_satuan_terkecil = st.id
...
function info_stock_opname_muat_data($idBarang = NULL, $startDate = NULL, $endDate = NULL) {
    ...
    $sql = "select s.waktu,s.id_packing_barang,s.sisa,sd.nama as sediaan,o.kekuatan,o.generik,ins.nama as pabrik,s.hpp,s.hna,b.nama as barang,pb.nilai_konversi,st.nama as satuan from stok_unit s
    left join packing_barang pb on s.id_packing_barang = pb.id
    left join barang b on pb.id_barang = b.id
    left join satuan st on pb.id_satuan_terkecil = st.id
    left join obat o on b.id = o.id
...
function sub_kategori_barang_muat_data($id_kategori = null) {
    ...
    $sql = "select s.*, k.nama as kategori from sub_kategori_barang s join kategori_barang k on (s.id_kategori_barang = k.id) where s.id_kategori_barang='".User::$pemesanan_barang_role."' $require order by s.nama asc";
    return _select_arr($sql);
}
...
function aturan_pakai_muat_list_data($id = null,$key=null) {
 if ($id != null) {
        $where = " where id = '$id'";
    }else
        $where='';
    ...
    $sql = "select * from aturan_pakai $where $cari order by nama asc";
  if ($id != null) {
    	$result = _select_unique_result($sql);
        $result['list'] = _select_arr($sql);
    }else
...
function cari_pembayaran_muat_data($norm=null,$id_penjualan=null){
    ...
    if($norm!=null){  
    $sql_a="SELECT
                bil.id as id_billing,bil.total_tagihan as total_tagihan_billing
            FROM pasien ps
...
function infobed_rawat_inap_muat_data($instalasi = NULL,$kelas = NULL){
    $return = array();
    $where = "";
    if($instalasi != NULL){
   ...
   if ($perundangan != NULL) {
    ...
    $sql = "
	 ...
	 left join obat o on b.id=o.id
            ...
            where kb.id='" . User::$pemesanan_barang_role . "' and skb.id=1 $where
    ";
    
    $return = _select_arr($sql);
    return $return;
}
function rawat_inap_muat_data($startDate = NULL,$endDate = NULL,$instalasi = NULL,$kelas = NULL){
    $return = array();
    $where = "";
    if ($startDate != NULL && $endDate != NULL) {
   ...
   if ($kelas != NULL) {
    ...
    foreach ($sql as $row){
        $sql2 = "
            select k.id as idKunjungan,k.waktu as tanggal,k.id_pasien as norm,pd.nama as pasien,dp.alamat_jalan,dokter.nama as nama_dokter,
...
function detail_rawat_inap($id = NULL){
    ...
    $sql = "
        select k.waktu as tanggal,k.id_pasien as norm,b.nama as bed,b.id as idBed,k.*,pd.nama as pasien,dp.alamat_jalan,dokter.nama as nama_dokter,dokter.id as idDokter,kel.nama as kelurahan,kls.nama as kelas,l.id as idLayanan,k.rencana_cara_bayar as paying,ins.nama as instalasi from kunjungan k
        left join pasien ps on k.id_pasien = ps.id
        left join penduduk pd on ps.id_penduduk = pd.id
        left join dinamis_penduduk dp on dp.id_penduduk = pd.id
	LEFT JOIN penduduk dokter on dokter.id=k.id_penduduk_dpjp
...
function stok_barang_pelayanan_muat_data($idPacking=null,$idPabrik=null,$idSubKategori=null,$getUnit=NULL){
    ...
    $sql = "select pb.id as packing_barang from stok_unit s
            left join packing_barang pb on s.id_packing_barang = pb.id
            left join unit u on s.id_unit = u.id
            left join barang b on pb.id_barang = b.id
            left join instansi_relasi ir on b.id_instansi_relasi_pabrik = ir.id
            left join sub_kategori_barang skb on b.id_sub_kategori_barang = skb.id
            where skb.id_kategori_barang='".User::$pemesanan_barang_role."' $where
            group by pb.id order by s.id desc,s.waktu desc";
    ...
    foreach ($query as $row){
        $result[] = _select_unique_result("select st.nama as satuan,ir.nama as pabrik,o.kekuatan,stok.id_packing_barang,
                ...
                (select st.hpp from stok_unit st where st.id_packing_barang=pb.id and st.id_jenis_transaksi='2' order by st.waktu desc limit 0,1) as hpp
                from stok_unit stok
            ...
            left join instansi_relasi ir on ir.id=b.id_instansi_relasi_pabrik 
            where pb.id = '$row[packing_barang]' order by stok.id desc,stok.waktu desc limit 0,1");
    }
    return $result;
}
function stok_barang_gudang_muat_data($idPacking=null,$idPabrik=null,$idSubKategori=null){
    ...
    $group="select stok.batch,stok.id_packing_barang from stok
            ...
            join barang b on b.id=pb.id_barang
            join sub_kategori_barang skb on skb.id=b.id_sub_kategori_barang
            where skb.id_kategori_barang='".User::$pemesanan_barang_role."' and stok.sisa > 0 $where
            group by stok.id_packing_barang,stok.batch";
    ...
    foreach($group as $a){
        ...
        stok.ed, stok.batch,stok.sisa,st.nama as satuan,st12.nama as kemasan,pb.nilai_konversi,stok.hpp,stok.hna,
        ...
        (select stok.hpp from stok where stok.id_packing_barang=pb.id and stok.id_jenis_transaksi='2' order by stok.waktu desc limit 0,1) as hpp
            ...
            and stok.id=(select max(st12.id) from stok st12 where st12.batch=stok.batch and st12.id_packing_barang=pb.id)
        limit 0,1";
//        echo $sql;
...
function last_detail_billing($id_billing,$waktu){
    $sql = "select db.id_tarif from detail_billing db join billing b on db.id_billing = b.id where b.id = '$id_billing' and db.waktu = '$waktu' order by db.id desc limit 0,1";
    return _select_unique_result($sql);
}
...
function distribusi_muat_data_by_id($id = NULL){
    ...
    $detail = "select b.nama as barang,st.nama as satuan,sd.nama as sediaan,ins.nama as pabrik,o.generik,o.kekuatan,pb.nilai_konversi,detail.jumlah_distribusi from detail_distribusi_penerimaan_unit detail
               left join packing_barang pb on detail.id_packing_barang = pb.id
               left join satuan st on pb.id_satuan_terkecil = st.id
               left join barang b on pb.id_barang = b.id
               left join instansi_relasi ins on b.id_instansi_relasi_pabrik = ins.id
...
function billing_detail_muat_data($id){
    $result['master']=
    _select_unique_result("select pasien.id as norm,dp.alamat_jalan as alamat,k.nama as kelurahan,kb.nama as kabupaten,bil.id,pdd.nama as penduduk 
         from billing bil
            join pasien on pasien.id=bil.id_pasien
            join penduduk pdd on pdd.id=pasien.id_penduduk
            join dinamis_penduduk dp on (dp.id_penduduk=pdd.id and dp.id=(SELECT max(id) FROM dinamis_penduduk where dinamis_penduduk.id_penduduk=pdd.id and dinamis_penduduk.akhir=1))
...
function data_tarif($code=NULL,$page=NULL,$data_perpage=NULL,$key=NULL){
	$result = array();
	$noPage = 1;
	$where="";
    $batas = "";
    ...
    $sql = "select * from kategori_tarif $where";

    if ($code != null) {
       $result = _select_unique_result($sql.$batas);
        $result['list'] = _select_arr($sql.' order by nama ASC '.$batas);
    } else {
        $result['list'] = _select_arr($sql.' order by nama ASC '.$batas);
    }
